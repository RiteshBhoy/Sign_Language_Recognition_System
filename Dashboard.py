__author__ = 'Mithlesh , Ravi Verma, Ritesh Bhoy , Dipanshu'
...
class Dashboard(QtWidgets.QMainWindow):
	def __init__(self):
		...
		# Capture frame-by-frame 
				...
				cv2.resizeWindow("mask",938,748)
				cv2.moveWindow("mask", 490,102)
		...
		self.label_3.setGeometry(0,160,941,520)
		movie.start()
	...
	def createGest(self):
		...
		while True:
			...
			try:
				frame=cv2.resize(frame,(321,321))
			...
			cv2.setWindowProperty("mask",cv2.WND_PROP_FULLSCREEN,cv2.WINDOW_FULLSCREEN)
			cv2.resizeWindow("mask",305,215)  ##cam image capture resize
			cv2.moveWindow("mask", 975,385)
	...
	def scanSent(self):
		...
		while True:
			...
			frame = cv2.flip(frame,1)
			try:
				frame=cv2.resize(frame,(331,377))
			...
			cv2.setWindowProperty("mask",cv2.WND_PROP_FULLSCREEN,cv2.WINDOW_FULLSCREEN)
			cv2.resizeWindow("mask",188,138) ## sentence read cam
			cv2.moveWindow("mask", 1184,380)
	...
	def scanSingle(self):
		...
		while True:
			...
			frame = cv2.flip(frame,1)
			try:
				frame=cv2.resize(frame,(321,340))
				frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
			...
			cv2.imshow("mask", mask)
			cv2.setWindowProperty("mask",cv2.WND_PROP_FULLSCREEN,cv2.WINDOW_FULLSCREEN)
			cv2.resizeWindow("mask",128,118)
			cv2.moveWindow("mask", 1190,390)
